...
public class PrivateMessagingPacketHandler implements PacketHandler {
    ...
    public static final int PMSTATUS = 95;     // CONVERTED
	...
	public void handlePacket(Player player, Packet packet) {
		...
		case PMSTATUS:
			...
			int b = packet.getIn().readByte();
			player.setPMStatus(a);
			//player.getActionSender().sendMessage("PmStatus: "+ player.getPMStatus() + " " + b+ " " + a);
			//int c = packet.getIn().readByte();
			//for (Player players : World.getPlayers()) {
			//	if (players != null){
					//players.getPrivateMessaging().updatePM(player.getIndex(), 1);
					//players.getPrivateMessaging().checkOnlineStatus(player);
			//	}
			//}
			break;
		case SEND_PM:
