...
public class ItemPacketHandler implements PacketHandler {
	...
	private void handleDropItem(Player player, Packet packet) {
		...
		/*if (item.getDefinition().isUntradable() && !item.getDefinition().getName().toLowerCase().contains()) {
	...
	private void handlePickupItem(Player player, Packet packet) {
		...
		player.setClickZ(player.getPosition().getZ());
		if(player.getClickId() == 245 && player.getClickX() == 2930 && player.getClickY() == 3515 && player.zammyFlameDelay <= 0){
			player.zammyFlameDelay = 4;
			player.getUpdateFlags().sendGraphic(78);
			player.hit(Misc.random(5), HitType.NORMAL);
			//int statLower = Misc.random(5);
			// TODO: Stat reducing
				
        	//addEffects(new Effect[]{new StatEffect(statLower, 3)});
           // setHits(new HitDef[]{hitDef.randomizeDamage().applyAccuracy().addEffects(new Effect[]{spell.getRequiredEffect(), spell.getAdditionalEffect()}).setUnblockable(unblockable)});
			for (final Npc n : World.getNpcs()) {
				if (n == null)
					continue;
				if (!n.isDead() && n.getMaxHp() > 0 && !n.isAttacking() && Misc.goodDistance(n.getPosition().getX(), n.getPosition().getY(), player.getPosition().getX(), player.getPosition().getY(), 3)) {
					n.getUpdateFlags().sendForceMessage("How dare you steal from Zamorak!");
		...
		if(player.getClickId() == 1583){
			if(player.getEquipment().getItemContainer().get(Constants.HANDS) == null){
				player.getActionSender().sendMessage("You need ice gloves to pick up the feather.");
				return;
			}
			if(player.getEquipment().getItemContainer().get(Constants.HANDS).getId() != 1580){
				player.getActionSender().sendMessage("You need ice gloves to pick up the feather.");
				return;
			}
		}
