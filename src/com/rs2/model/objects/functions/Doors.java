...
public class Doors {
	...
	public static boolean handleDoor(int id, int x, int y, int z) {
		...
		if (!objectName.contains("wall") && !objectName.contains("fence") && !objectName.contains("door") || objectName.contains("trapdoor") || objectName.contains("tree")) {
			return false;
		}
		...
		if (id == 1513) {
			d.doorId = 1534;
		} else if (id == 22 || id == 2025 || id == 2550 || id == 2551 || id == 2556 || id == 2558 || id == 2557 || id == 3014) {
			d.doorId = id;
		...
		ObjectHandler.getInstance().removeObject(d.doorX, d.doorY, d.doorZ, 0);
		if((id == 14749 || id == 14750) && y == 3935){
			new GameObject(Constants.EMPTY_OBJECT, d.doorX, d.doorY, d.doorZ, -1, d.type, Constants.EMPTY_OBJECT, 999999999, false);
		}else{
			new GameObject(d.doorId, d.doorX, d.doorY, d.doorZ, newFace, d.type, Constants.EMPTY_OBJECT, 999999999);
		}
		return true;
	}
	...
	public static void handlePassThroughDoor2(final Player player, final int id, final int x, final int y, final int z, final int destX, final int destY) {
		Doors doors = getDoor(id, x, y, z);
		if (doors == null) {
			player.setStopPacket(false);
			return;
	...
	private static int[] openDoors = {1504, 1514, 1517, 1520, 1531, 1534, 2033, 2035, 2037, 2998, 3271, 4468, 4697, 6101, 6103, 6105, 6107, 6109, 6111, 6113, 6115, 6976, 6978, 8696, 8819, 10261, 10263, 10265, 11708, 11710, 11712, 11715, 11994, 12445, 13002,14750};
}
