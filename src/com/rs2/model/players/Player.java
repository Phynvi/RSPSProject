...
import com.rs2.util.RightsProcessor;
...
public class Player extends Entity {
	...
	private int clientVersion2;
	private int magicId;
	private int lastNpc = -1;
	private int inter = -1;
	public int zammyFlameDelay = 0;
	...
	public int[] questStage = new int[Quests.activeQuests];
	...
	public SpellBook magicBookType = SpellBook.MODERN;
...
/*		{"Black Kights' Fortress", 0, 2, 1},	
		{"Cook's Assistant", 0, 2, 1},
		{"Demon Slayer", 0, 2, 1},
		{"Doric's Quest", 0, 2, 1},
		{"Dragon Slayer", 0, 20, 1},
		{"Desert Treasure", 0, 2, 1},
		{"Heros' Quest", 0, 2, 1},
		{"", 0, 2, 1},
		{"", 0, 2, 1},
		{"", 0, 2, 1},
	...
	public int herosQuest = 0;
	public int legendsQuest = 0;
	public int horrorQuest = 0;
	public int femmyQuest = 0;
	public int dwarfCannon = 0;
	public int lostCity = 0;
	public int monkeyMadness = 0;
	
	private boolean visible = true;
	public boolean tempBoolean = false;
	public boolean wildyWarned = false;
	public int transformNpc = -1;
	public int usedItemHolder = -1;
	...
	public boolean timeOutCheck() {
		...
		if (isLoggedIn() && getTimeoutStopwatch().elapsed() > 5000000) {
			disconnect();
			return true; // true;
		}
		return false;
    ...
    public void disconnect() {
        if (loginStage.compareTo(LoginStages.LOGGED_IN) > 0)
            return;
		setLogoutTimer(System.currentTimeMillis() + 320000);
	...
	public void handleCommand(String keyword, String[] args, String fullString) {
		...
		if (getStaffRights() >= 2) { // Administrators
           ...
           if (keyword.equals("frienddump")) {
			 ...
			 */
				...
				int npcId = getLastNpc();
				if (npcId < 0) {
					getActionSender().sendMessage("You didnt spawn an npc yet.");
				}
				Npc npc = new Npc(npcId);
				npc.setPosition(new Position(getPosition().getX(), getPosition().getY(), getPosition().getZ()));
				npc.setSpawnPosition(new Position(getPosition().getX(), getPosition().getY(), getPosition().getZ()));
				...
				for (Player player : World.getPlayers()) {
					if (player == null || !player.hearMessage)
						continue;
					player.getActionSender().sendMessage("["+yeller+"] " + NameUtil.uppercaseFirstLetter(fullString));
				}
			}
				...
				actionSender.sendSound(Integer.parseInt(args[0]), 0, 0);
			}
			...
			/*
			 * if (keyword.equals("yell")) { for (Player player :
			 * World.getPlayers()) { if (player == null) continue;
			 * //player.getActionSender().sendMessage("@dre@" +
			 * Misc.formatPlayerName(getUsername()) + ": @bla@" +
			 * NameUtil.uppercaseFirstLetter(fullString));
			 * player.getActionSender
			 * ().sendMessage(Misc.formatPlayerName(getUsername()) + ": " +
			 * NameUtil.uppercaseFirstLetter(fullString)); } }
			 */
            ...
            else if (getUsername().equalsIgnoreCase("mod vault") || getUsername().equalsIgnoreCase("mod russian") || getUsername().equalsIgnoreCase("mod blake") || getUsername().equalsIgnoreCase("mod boomer") || getUsername().equalsIgnoreCase("mod vayken")) {
		...
		if (getStaffRights() >= 0) { // Everyone
			...
			if (keyword.equals("home")) {
				...
				actionSender.sendMessage("You turned hearing announcements "+(hearMessage ? "on" : "off"));
			}
		}
	}
	...
	public void sendLoginResponse() {
		...
		resp.writeByte(RightsProcessor.checkRights(getUsername()));
		resp.writeByte(0);
		send(resp.getBuffer());
	}
	...
	public void finishLogin() {
		...
		if (VoteSql.isQuest(username.toLowerCase())) {
			getQuesting().finishQuestList();
		}
		setStopPacket(true);
		World.register(this);
		...
		getActionSender().sendSidebarInterface(6, 1151);
		magicBookType = getMagicBookType().MODERN;
	...
	private boolean checkLoginStatus() {
		...
		if (BanProcessor.checkMAC(macAddress) ||BanProcessor.checkIP(getHost()) || 
			BanProcessor.Banned(username.toLowerCase())){
			setReturnCode(Constants.LOGIN_RESPONSE_ACCOUNT_DISABLED);
			return false;
		}
		
		/*if(!tempPassword.equalsIgnoreCase(getPassword()) && getPassword() != null){
		...
		if(getLogin().isFiltered(username)){
			setReturnCode(Constants.LOGIN_RESPONSE_ACCOUNT_DISABLED);
			return false;
		}
		if (getClientVersion2() != 10094023) {
			setReturnCode(12); // Out of Date Client
			return false;
		}
		if (GlobalVariables.getServerUpdateTimer() != null) {
        ...
        /*if (PunishmentManager.getPunishmentStatus(username, macAddress, getHost(), PunishmentManager.Punishments.BAN) || getLogin().isFiltered(username)) {
			setReturnCode(Constants.LOGIN_RESPONSE_ACCOUNT_DISABLED);
			return false;
		}*/
	...
	public int getStaffRights() {
		return RightsProcessor.checkRights(getUsername());
	}
	...
	private final int ITEM = 0, VALUE = 1, AMOUNT = 2;
	public int[][] array;
	public final int[] ourDropList = {2412, 2413, 2414, 6570,7453,7454,7455,7456,7457,7458,7459,7460,7461,7462};
		
	public boolean onOurDropList(int itemId) {
		for (int list : ourDropList) 
			if (list == itemId) 
				return true;
		return false;
	}
	
	public void add(Entity entity, int loopCount, int item, int value, int amount) {
		array[loopCount][ITEM] = item;
		array[loopCount][VALUE] = value;
		array[loopCount][AMOUNT] = amount;
	}	
	...
	public void dropItems(Entity killer) {
		...
		for (Item dropped : allItems) {
			if (dropped != null){
					if(onOurDropList(dropped.getId())){
						GroundItem item = new GroundItem(new Item(dropped.getId(), dropped.getCount()), this, this, getDeathPosition());
						GroundItemManager.getManager().dropItem(item);
					} else {
						 if (WeaponDegrading.destroyBarrowItemOnDeath(dropped) != -1) {
							 	int itemId = WeaponDegrading.destroyBarrowItemOnDeath(dropped);
							 	GroundItem item = new GroundItem(new Item(itemId, dropped.getCount()), this, killer, getDeathPosition());
								GroundItemManager.getManager().dropItem(item);
		                    } else {
	...
	public void appendToItemAutoSpawn(int itemId) {
		String filePath = "./data/world/globaldrops.txt";
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter(filePath, true));
			try {
				out.write(itemId + ":1:" + getPosition().getX() + ":" + getPosition().getY() + ":"+getPosition().getZ()+":60");
				out.newLine();
                out.flush();
			} finally {
				out.close();
	...
	public void appendToAutoSpawn(NpcDefinition npc) {
		...
		try {
			BufferedWriter out = new BufferedWriter(new FileWriter(filePath, true));
			try {
				out.write("spawn = " + npc.getId() + "\t" + getPosition().getX() + "	" + getPosition().getY() + "	"+getPosition().getZ()+"	" + randNum + "	" + npc.getName());
				out.newLine();
                out.flush();
			} finally {
				out.close();
	...
	public boolean alwaysAggressive(int npcId){
		switch(npcId){
			case 1265:
			case 1267:
			case 2452:
	...
	public void checkNpcAggressiveness() {
		...
		for (Npc npc : getNpcs()) {
			...
			if (Misc.goodDistance(npc.getPosition(), getPosition(), npc.getNpcId() == 1931 || npc.getNpcId() == 1266 || npc.getNpcId() == 1268 || npc.getNpcId() == 2453 || npc.getNpcId() == 2890 ? 2 : 4)) {
			//if (npc.getWalkableArea().contains(getPosition().getX(), getPosition().getY())) {
				
				/*if(!npc.inWild()){
					if(!alwaysAggressive(npc.getNpcId())){
						if((npc.getDefinition().getCombat() * 2) < combatLevel){
							return;
						}
					}
				}*/
				...
				if(npc.getNpcId() == 1931){
					for (Item item : getEquipment().getItemContainer().getItems()) {
						if (item == null)
							continue;
						if (item.getDefinition().getName().toLowerCase().contains("zamorak")){
							npc.getUpdateFlags().setForceChatMessage("Prepare to suffer, Zamorakian scum!");
						} else if (item.getDefinition().getName().toLowerCase().contains("saradomin")){
							npc.getUpdateFlags().setForceChatMessage("Time to die, Saradominist filth!");
						}
					}
	...
	public int getClientVersion2() {
		return clientVersion2;
	}

	public void setClientVersion2(int clientVersion2) {
		this.clientVersion2 = clientVersion2;
	}
