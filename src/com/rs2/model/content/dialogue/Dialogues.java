...
public class Dialogues {
	...
	public static boolean sendDialogue(Player player, int id, int chatId, int optionId, int npcChatId) {
		...
		switch(id) {
			case 1918:
				if(player.getQuesting().getQuestStage(25) <= 0){
					switch(player.getDialogue().getChatId()) {
						case 1:
							player.getDialogue().sendNpcChat("Hello there adventurer!", CONTENT);
							return true;
					}
				}
				break;
			case 796:
				if(player.getQuesting().getQuestStage(49) <= 0){
					switch(player.getDialogue().getChatId()) {
					case 1:
						player.getDialogue().sendNpcChat("Hello there adventurer!", CONTENT);
						return true;
					case 2:	
						player.getDialogue().sendPlayerChat("Can I join your guild?", HAPPY);
						return true;
					case 3:
						player.getDialogue().sendNpcChat("Well I do need some help gathering items.", CONTENT);
						return true;
					case 4:
						player.getDialogue().sendOption("Ok, what do you need me to get?", "No Thanks, not right now?");
						return true;
					case 5:
						switch(optionId) {
						case 1:
							player.getDialogue().sendNpcChat("I need a Firebird's Feather, Master Thief Arm Band","& Cooked Lava Eel", CONTENT);
							
							return true;
						case 2:
							player.getDialogue().sendNpcChat("Ok see you around!", CONTENT);
							player.getDialogue().endDialogue();
							return true;
					}
					break;
					case 6:
						player.getDialogue().sendPlayerChat("Sounds great be back soon!", HAPPY);
						player.getDialogue().endDialogue();
						player.getQuesting().setQuestStage(49, 1);
						return true;
					}
				}
				if(player.getQuesting().getQuestStage(49) == 1){
					if (player.getInventory().playerHasItem(1583) && player.getInventory().playerHasItem(1579) &&
							player.getInventory().playerHasItem(2149)){
						switch(player.getDialogue().getChatId()) {
							case 1:
								player.getInventory().removeItem(new Item(1583));
								player.getInventory().removeItem(new Item(2149));
								player.getInventory().removeItem(new Item(1579));
								player.getDialogue().dontCloseInterface();
								player.getQuesting().completeQuest(49, "1 Quest Point", "29,232 Experence spread over 12 skills", "The ability to wield Dragon mace & Battleaxe!","", "", 1434);
						        player.getSkill().addExp(Skill.ATTACK, 487);
						        player.getSkill().addExp(Skill.STRENGTH, 487);
						        player.getSkill().addExp(Skill.HITPOINTS, 487);
						        player.getSkill().addExp(Skill.RANGED, 487);
						        player.getSkill().addExp(Skill.MINING, 487);
						        player.getSkill().addExp(Skill.COOKING, 487);
						        player.getSkill().addExp(Skill.WOODCUTTING, 487);
						        player.getSkill().addExp(Skill.FIREMAKING, 487);
						        player.getSkill().addExp(Skill.SMITHING, 487);
						        player.getSkill().addExp(Skill.DEFENCE, 487);
						        player.getSkill().addExp(Skill.HERBLORE, 487);
						        player.getSkill().addExp(Skill.FISHING, 487);
								player.getDialogue().dontCloseInterface();
								player.getQuesting().setQuestStage(49, 6);
								return true;
						}
						...
						switch(player.getDialogue().getChatId()) {
							case 1:
								player.getDialogue().sendNpcChat("Come back with all the items", CONTENT);
								return true;
						}
					}
				}
			...
			case 519:
				switch(player.getDialogue().getChatId()) {
					case 1:
						if(player.usedItemHolder > -1){
							player.getDialogue().sendOption("Can you fix my item.", "Can I see your store!", "Nevermind");
						} else {
							player.getDialogue().sendOption("Can I see your store!", "Nevermind");
					...
					case 2:
						switch(optionId) {
							case 1:
								player.getDialogue().sendNpcChat("That will cost you "+ Barrows.getRepairPrice(player.usedItemHolder) + " Gp.", CONTENT);
								return true;
							case 2:
								player.getDialogue().sendPlayerChat("Can I see your store!", HAPPY);
						...
						break;
					case 3:
						if(player.getInventory().playerHasItem(995, Barrows.getRepairPrice(player.usedItemHolder))){
							player.getDialogue().sendOption("Ok fix it please!", "Nevermind!");
							return true;
						} else {
							player.getDialogue().sendPlayerChat("I don't have that much!", SAD);
							player.getDialogue().endDialogue();	
							player.usedItemHolder = -1;
							return true;
						}
					case 4:
						switch(optionId) {
							case 1:
								if(player.usedItemHolder > -1 && Barrows.getRepairPrice(player.usedItemHolder) > -1){
									if(player.getInventory().playerHasItem(995, Barrows.getRepairPrice(player.usedItemHolder))){
										if(player.getInventory().playerHasItem(player.usedItemHolder)){
											player.getInventory().removeItem(new Item(995, Barrows.getRepairPrice(player.usedItemHolder)));
											player.getInventory().replaceItemWithItem(new Item(player.usedItemHolder), new Item(Barrows.getRepairedItem(player.usedItemHolder)));
											player.getDialogue().endDialogue();
											player.usedItemHolder = -1;
											return true;
										} else {
											player.usedItemHolder = -1;
											player.getDialogue().sendPlayerChat("I don't have that item anymore!", SAD);
											player.getDialogue().endDialogue();
											return true;
									...
									player.usedItemHolder = -1;
									player.getDialogue().sendNpcChat("You need to use the correct item on me first.", SAD);
									player.getDialogue().endDialogue();
									return true;
								}
							case 2:
						...
						break;
					case 5:
						ShopManager.openShop(player, 30);
						player.getDialogue().dontCloseInterface();
						break;
			...
			break;
			case 650:
				if(player.getQuesting().getQuestStage(57) <= 0){
					switch(player.getDialogue().getChatId()) {
						case 1:
							player.getDialogue().sendNpcChat("Hello there adventurer!", CONTENT);
							return true;
						case 2:	
							player.getDialogue().sendOption("What are you doing out here?", "Know any adventures I can go on?");
							return true;
						case 3:
							switch(optionId) {
								case 1:
									player.getDialogue().sendNpcChat("We are on a quest on our own!", CONTENT);
									player.getDialogue().endDialogue();
									return true;
								case 2:
									player.getDialogue().sendNpcChat("No, This is our quest. We will find the lost ci...", CONTENT);
									
									return true;
							}
							break;
						case 4:
							player.getDialogue().sendPlayerChat("Wait! What is the lost city?", CONTENT);
							return true;
						case 5:
							player.getDialogue().sendNpcChat("Nothing, we will find it first!", CONTENT);
							return true;
						case 6:
							player.getDialogue().sendNpcChat("All we know is it involves a special branch", "and a shed in lumbridge.", CONTENT);
							return true;
						case 7:
							player.getDialogue().sendPlayerChat("Hmmm, so that's it?", CONTENT);
							return true;
						case 8:
							player.getDialogue().sendNpcChat("That is all I know....", CONTENT);
							player.getDialogue().endDialogue();
							player.getQuesting().setQuestStage(57, 1);
							return true;
			...
			break;
			case 747:
				if(player.getQuesting().getQuestStage(4) == 6){
					switch(player.getDialogue().getChatId()) {
						case 1 :
				...
				} else if(player.getQuesting().getQuestStage(4) <= 0){
					player.getDialogue().sendNpcChat("Please go see the Champions Guild Master", CONTENT);
					player.getDialogue().dontCloseInterface();
					break;
					
				}else if(player.getQuesting().getQuestStage(4) == 1){
					switch(player.getDialogue().getChatId()) {
						case 1:					
							player.getDialogue().sendPlayerChat("I am looking to obtain a rune platebody.", DISORIENTED_RIGHT);
							return true;
						case 2:
							player.getDialogue().sendNpcChat("You must first slay the mighty Elvarg on Crandor", CONTENT);
							player.getQuesting().setQuestStage(4, 2);
							return true;
						case 3:
							player.getDialogue().sendPlayerChat("Sounds easy enough to me!", HAPPY);
							player.getDialogue().dontCloseInterface();
							player.getQuesting().setQuestStage(4, 2);
							return true;
						case 4:
							player.getDialogue().sendPlayerChat("Now I have to figure out how to get there.", SAD);
							player.getDialogue().dontCloseInterface();
							player.getQuesting().setQuestStage(4, 2);
							return true;
					}
					
				} else if(player.getQuesting().getQuestStage(4) == 5 || player.getQuesting().getQuestStage(4) == 4 || player.getQuesting().getQuestStage(4) == 3){
					player.getQuesting().completeQuest(4, "2 Quest Points", "18650 Strength Experience", "18650 Defence Experience", "The ability to wear Rune & Dragon Bodys!", "", 1127);
			        player.getSkill().addExp(Skill.DEFENCE, 3730);
			        player.getSkill().addExp(Skill.STRENGTH, 3730);
					player.getDialogue().dontCloseInterface();
					player.getQuesting().setQuestStage(4, 6);
					break;
				}	
			break;
			case 198:
				switch(player.getDialogue().getChatId()) {
				case 1 :
					if(player.getQuesting().getQuestStage(4) > 0){
						player.getDialogue().sendNpcChat("You are already in the middle of dragon slayer", CONTENT);
						player.getDialogue().endDialogue();
						return true;
					}	
					player.getDialogue().sendNpcChat("Would you like a quest?", CONTENT);
					return true;
				case 2 :
					player.getDialogue().sendOption("Yes please.", "No thanks.");
					return true;
				case 3 :
					switch(optionId) {
						case 1:
							player.getDialogue().sendNpcChat("Go talk to Oziach he gives quests to true champions.", CONTENT);
							player.getDialogue().dontCloseInterface();
							player.getQuesting().setQuestStage(4, 1);
							break;
						case 2:
					...
					case 3:
						switch(optionId) {
							case 1:
								...
								return true;
							case 2:
								player.getDialogue().sendNpcChat("Aye, not too bad thank you. Lovely weather", "in Near Reality this fine day.", HAPPY);
			...
			case 381:
				switch(player.getDialogue().getChatId()) {
				case 1 :
					player.getDialogue().sendNpcChat("Would you like to sail to Brimhaven?", "It will only cost you 30 coins.", CONTENT);
					return true;
				case 2 :
				...
				case 3 :
					...
					break;
				case 4 :
					player.getDialogue().sendNpcChat("So do you have the 30 coins for my service,", "and are you ready to leave now?", CONTENT);
					player.getDialogue().setNextChatId(3);
					break;
			}
			break;
			case 744 : //Klarese to Crandor
				switch(player.getDialogue().getChatId()) {
					case 1 :
						player.getDialogue().sendNpcChat("Would you like to sail to Crandor Island?", "It will only cost you 1000 coins.", CONTENT);
						return true;
					case 2 :
						player.getDialogue().sendOption("YES", "NO");
						return true;
			...
			case 1304 : //jarvald
				switch(player.getDialogue().getChatId()) {
					case 1 :
						player.getDialogue().sendNpcChat("Would you like to sail to Miscellania Island?", CONTENT);
						return true;
					case 2 :
						player.getDialogue().sendOption("YES", "NO");
						return true;
					case 3 :
						switch(optionId) {
							case 1:
								Sailing.sailShip(player, ShipRoute.RELLEKKA_TO_MISC);
								player.getDialogue().dontCloseInterface();
								break;
						}
						break;
			...
			case 1385 : //jarvald
				switch(player.getDialogue().getChatId()) {
					case 1 :
						player.getDialogue().sendNpcChat("Would you like to sail to Rellekka Island?", CONTENT);
						return true;
					case 2 :
						player.getDialogue().sendOption("YES", "NO");
						return true;
					case 3 :
						switch(optionId) {
							case 1:
								Sailing.sailShip(player, ShipRoute.MISC_TO_RELLEKKA);
								player.getDialogue().dontCloseInterface();
								break;
						}
						break;
			...
			case 380 : //karamja sailor
				if(player.getPosition().getY() == 3147 || player.getPosition().getY() == 3146){
					switch(player.getDialogue().getChatId()) {
						case 1 :
				...
				if(player.getPosition().getX() == 2771 || player.getPosition().getX() == 2772){
					switch(player.getDialogue().getChatId()) {
						case 1 :
			...
			case 947 : //financial instructor
				switch(player.getDialogue().getChatId()) {
					...
					case 6 :
						...
						return true;
					case 7 :
						player.getDialogue().sendNpcChat("Now, the next way to earn money quickly is by quests.", "Many people on here have things they need", "doing, which they will reward you for.", HAPPY);
						return true;
					case 8 :
			...
			case 946 : //magic instructor
				switch(player.getDialogue().getChatId()) {
					...
					case 10 :
						...
						return true;
					case 11 :
						player.getDialogue().sendNpcChat("If all else falls, visit the website for a whole", "board of information on quests, skills and minigames", "as well as a very good starter's guide", HAPPY);
						return true;
					case 12 :
			...
			case 70 : //slayer masters
				...
				case 1 :
					...
					return true;
				case 2 :
					player.getDialogue().sendOption("I need another assignment", "Do you have anything for trade?", "Can I change my task?", "Er...nothing");
					return true;
				case 3 :
					switch(optionId) {
						case 1 :
						...
						case 2 :
							...
							return true;
						case 3 :
							player.getDialogue().sendNpcChat("Sure!", Dialogues.HAPPY);
							player.getDialogue().setNextChatId(7);
							return true;	
					}
					break;
