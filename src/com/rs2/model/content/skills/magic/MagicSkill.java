...
public abstract class MagicSkill extends CycleEvent {
		...
		MagicSkill magicSkill = new MagicSkill(player, spell) {
			...
			public boolean onExecute() {
				switch (spell) {
					...
					case EDGEVILLE2:
						if (player.inWild()) {
							player.getActionSender().sendMessage("You can't teleport from the wild using home tele.");
							return false;
						}
					...
					case APE_ATOLL :
						...
						player.getActionSender().sendMessage("You can't teleport to here yet.");
						return false;
					case PADDEWWA :
	...
	public boolean teleOther(Player player, Player otherPlayer, TeleotherLocation location) {
		...
		if (otherPlayer.inWild() && otherPlayer.getWildernessLevel() > 20) {
			player.getActionSender().sendMessage("You can't teleport this person above 20 wild.");
			return false;
		}
