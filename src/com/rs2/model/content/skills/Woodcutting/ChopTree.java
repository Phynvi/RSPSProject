...
public class ChopTree {
	...
	public static void handle(final Player player, final int id, final int x, final int y) {
		...
		if (tree == Tree.DRAMEN_TREE && !player.hasKilledTreeSpirit()) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (Misc.random(350) == 0) { // Birds nest (chance = 281)
                    GroundItem item = new GroundItem(new Item(5073 + Misc.random(1)), player);
                    GroundItemManager.getManager().dropItem(item);
					player.getActionSender().sendMessage("A nest has fallen out of the tree!");
				}
