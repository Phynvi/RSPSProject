...
public class CombatManager extends Tick {
	...
	public static void attack(Entity attacker, Entity victim) {
      ...
      /* if(attacker.inMageArena()){
        	if(attacker.isPlayer() && (attacker.getAttackType() != Entity.AttackTypes.MAGIC)){
        		((Player) attacker).getActionSender().sendMessage("You cannot melee or ranged in mage arena." + attacker.getAttackType() );
        		CombatManager.resetCombat(attacker);
        		return;
        	}
        }*/
        if(victim.isNpc() && ((Npc) victim).getNpcId() == 742){
        	if(attacker.isPlayer()){
        		if(((Player) attacker).getQuesting().getQuestStage(4) < 2 || ((Player) attacker).getQuesting().getQuestStage(4) == 6){
        ...
        if(victim.isNpc() && ((Npc) victim).getNpcId() == 742){
        	if(attacker.isPlayer()){
        		if(((Player) attacker).getQuesting().getQuestStage(4) == 6){
        			((Player) attacker).getActionSender().sendMessage("You cannot attack this npc.");
        			CombatManager.resetCombat(attacker);
                	return;
        		}
		...
		/* if(victim.isNpc() && ((Npc) victim).getNpcId() == 1615){
			 int newSpot = Misc.random(6);
			 int pX = attacker.getPosition().getX();
			 int pY = attacker.getPosition().getY();
			 final int pZ = attacker.getPosition().getZ();
			 if(newSpot == 0){
				 pX = pX +1;
				 victim.getUpdateFlags().sendAnimation(714, 0);
				 victim.getUpdateFlags().sendGraphic(301, 100 << 16);
			 ...
			 if(newSpot == 5){
				 ...
				 pX = victim.getPosition().getX();
				 pY = victim.getPosition().getY();
			 }
			 final int pX2 = pX;
			 final int pY2 = pY;
			 final Entity victim2 = victim;
				CycleEventHandler.getInstance().addEvent(victim2, new CycleEvent() {
					int teleTimer = 6;
					@Override
					public void execute(CycleEventContainer container) {
						teleTimer--;
						if (teleTimer == 2) {
								victim2.getUpdateFlags().sendAnimation(1534);
								victim2.teleport(new Position(pX2, pY2, pZ));
						}
						if (teleTimer < 1) {
							container.stop();
						}
	...
	public static void endDeath(final Entity died, final Entity killer, final boolean firstTime) {
		if (firstTime) {
			/*if(died.isPlayer() && killer.isPlayer()){
				...
				victim.dropItems(attacker, true);
			} else {*/
        ...
        if (died.isNpc()) {
    		...
    		final Npc npc = (Npc) died;
    		if(killer.isPlayer() && ((Player) killer).getQuesting().getQuestStage(4) == 2 && npc.getNpcId() == 742){
				((Player) killer).getQuesting().setQuestStage(4, 5);
        		
    		}
    		if(killer.isPlayer() && ((Player) killer).getQuesting().getQuestStage(4) == 3 && npc.getNpcId() == 742){
				((Player) killer).getQuesting().setQuestStage(4, 5);
        		
    		}
    		if(killer.isPlayer() && ((Player) killer).getQuesting().getQuestStage(4) == 4 && npc.getNpcId() == 742){
				((Player) killer).getQuesting().setQuestStage(4, 5);
	...
	public static double getDefenceRoll(Entity victim, HitDef hitDef) {
		...
		effectiveDefence *= (1 + (styleBonusDefence) / 34);
	...
	public static double getAttackRoll(Entity attacker, HitDef hitDef) {
		...
		if (attackStyle.getMode() == AttackStyle.Mode.MELEE_ACCURATE || attackStyle.getMode() == AttackStyle.Mode.RANGED_ACCURATE)
			...
			styleBonusAttack = 1;
		effectiveAccuracy *= (1 + (styleBonusAttack) / 94);//64
